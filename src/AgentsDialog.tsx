...
export function AgentsDialog({onClose}:{onClose:()=>void}){
 ...
 const refresh=()=>{setChecking(true);getSkillStatus().then(setStatus).catch(()=>setStatus(null)).finally(()=>setChecking(false))};
 useEffect(refresh,[]);
 return <div className="verdict-backdrop" onClick={onClose}>
  <div className="verdict-card agents-dialog" onClick={e=>e.stopPropagation()}>
   ...
   <div className={`skill-status ${status?.installed?'ok':'warn'}`}>
    {status===null||checking?<>Checking for installed skills…</>
     :status.installed?<><CircleCheck size={15}/> Skill installed — detected for {status.locations.map(l=>l.agent).join(', ')}.</>
   ...
   {!status?.installed&&<p>Install the Margin skill so Claude Code, Cursor, Codex, and other agents can push any markdown file into Margin for guided review — and read back your notes, review answers, and argument maps.</p>}
