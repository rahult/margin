...
export function App(){
 ...
 const [active,setActive]=useState(saved?.active??0); const [mode,setMode]=useState<Mode>('read'); const [listening,setListening]=useState(false); const [note,setNote]=useState(''); const [notes,setNotes]=useState<string[]>(saved?.notes??[]); const article=useRef<HTMLElement>(null); const fileInput=useRef<HTMLInputElement>(null); const speechGen=useRef(0);
 ...
 const stopListening=()=>{speechGen.current++;narrator.stop();setListening(false)};
 ...
 const loadDoc=(d:ParsedDoc,md?:string)=>{stopListening();const s=loadProgress(d.title,d.sections.length);setDoc(d);setSaved(s);setActive(s?.active??0);setNotes(s?.notes??[]);setMode('read');setNote('');setQuestion('');setAnswer('');try{if(md&&md.length<1_500_000)localStorage.setItem(DOC_KEY,JSON.stringify({name:d.title,md}));else if(md===undefined)localStorage.removeItem(DOC_KEY)}catch{/* storage full — doc simply will not survive reload */}};
 ...
 return <div className={`app mode-${mode}`}>
  ...
  <main className="workspace">
   ...
   <section className="reader-shell"><div className="reader-toolbar"><div className="mode-tabs"><button className={mode==='read'?'active':''} onClick={()=>setMode('read')}>Read</button><button className={mode==='review'?'active':''} onClick={()=>setMode('review')}>Review</button></div><div><button className={`tool ${listening?'active':''}`} onClick={()=>listening?stopListening():setListening(true)}>{listening?<Square size={15}/>:<Headphones size={16}/>} {listening?'Stop':'Listen'}</button></div></div>
