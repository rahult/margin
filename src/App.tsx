...
import {BookOpen,Brain,Bird,ChevronRight,Focus,FolderOpen,Headphones,MessageSquareText,Plug,Settings,Square,Sparkles,Target,X} from 'lucide-react';
...
export function App(){
 ...
 const [ready,setReady]=useState(false); const [checking,setChecking]=useState(true); const [model,setModel]=useState(''); const [question,setQuestion]=useState(''); const [answer,setAnswer]=useState(''); const [asking,setAsking]=useState(false); const [showAgents,setShowAgents]=useState(false); const [showSettings,setShowSettings]=useState(false);
 ...
 return <div className={`app mode-${mode}`}>
  ...
  <header className="topbar"><div className="brand"><div className="mark">M</div><span>Margin</span><em>{model||'local proxy'}</em></div><div className="doc-title"><BookOpen size={15}/> {doc.title}</div><div className="top-actions"><WalletPill coins={coins}/>{doc!==defaultDoc&&<button className="tool" onClick={()=>loadDoc(defaultDoc)}><X size={15}/> Close</button>}<button className="tool" onClick={()=>fileInput.current?.click()}><FolderOpen size={15}/> Open</button><button className="tool" title="Connect coding agents" onClick={()=>setShowAgents(true)}><Plug size={15}/> Agents</button><button className="tool" title="Settings" onClick={()=>setShowSettings(true)}><Settings size={15}/></button><Button label={mode==='focus'?'Exit focus':'Focus'} variant="secondary" onClick={()=>setMode(mode==='focus'?'read':'focus')}/></div></header>
  ...
  {showSettings&&<SettingsDialog model={model} onClose={()=>setShowSettings(false)} onSaved={m=>setModel(m)}/>}
 </div>
}
